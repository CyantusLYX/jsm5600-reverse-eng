...
# Configure logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s - [SEM_BRIDGE] - %(message)s")
        ...
        if "groups" in self.definitions and opcode_hex in self.definitions["groups"]:
            ...
            if "matches" in group:
                for rule in group["matches"]:
                    ...
                    if matched:
                        return rule.get("name", "UnknownGroupCmd"), rule.get("level", "INFO")
            
            # If no specific match, return group name, default to WARN if unknown
...
class SCSILogger:
    ...
    def log_transaction(self, cdb_bytes, data_bytes, direction, status, cmd_name, defined_level="INFO"):
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        ...
        if status != 0 and status != 1: 
             if status == 4: level = "ERR "
             elif status != 1: level = "WARN"
    ...
    def handle_client(self, conn, addr):
        ...
        try:
            ...
            while True:
                ...
                session_logger.log_transaction(cdb, None, "CMD", 0, cmd_name, defined_level=cmd_level)

                # 3. Execute on Real Device
                resp_data, status = self.send_scsi_cmd(cdb)

                # Log Response
                session_logger.log_transaction(cdb, resp_data, "RES", status, cmd_name, defined_level=cmd_level)
